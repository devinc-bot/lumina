...
- H1: As an operator, I want the three frontends deployed to Cloudflare from the
  `staging` branch so that the staging release matches the API environment.
...
- RF-1: WHEN a successful CI run for `staging` is published, THE SYSTEM SHALL serve
...
- Use GitHub Actions after successful CI, aligned with the Cloud Run staging branch
  model.
...
- Publish after successful CI for the `staging` branch. Include all three frontends in
  this iteration.
