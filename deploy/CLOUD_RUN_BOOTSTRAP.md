...
# Verify ownership of lumina-events.com once in Google Search Console, then create the mapping.
# Add the DNS records printed by the describe command and wait for the managed certificate before
# configuring Google OAuth, Mercado Pago, or frontend VITE_API_URL.
gcloud beta run domain-mappings create `
  --project=$PROJECT_ID `
  --region=$REGION `
  --service=lumina-api-staging `
  --domain=api-staging.lumina-events.com
...
# Keep the public API origin for browser auth. The Scheduler target and audience stay on the
# native service URL, which avoids making internal jobs depend on the public DNS mapping.
$SERVICE_URL | gcloud secrets versions add lumina-staging-internal-jobs-oidc-audience `
  --project=$PROJECT_ID `
  --data-file=-
