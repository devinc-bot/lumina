...
`VITE_DASHBOARD_URL=https://staging-dash.lumina-events.com`. Set
`VITE_API_URL=https://api-staging.lumina-events.com` after the Cloud Run domain mapping is active.
...
the `staging` branch after `VITE_API_URL` is a healthy custom API URL. Then, in the
...
- `staging-dash.lumina-events.com` → `lumina-dashboard-staging`
